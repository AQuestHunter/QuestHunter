-- Archive quests after grace period: ends_at + 1 day → archived = true.
-- Players already lose access when ends_at passes; this marks rows for admin hygiene / retention.

alter table public.quests
  add column if not exists archived boolean not null default false;

create index if not exists quests_archived_idx on public.quests (archived);

-- Refresh player-facing summaries (hide archived even if mis-scheduled)
...
create or replace function public.run_quest_archive_sweep()
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  update public.quests
   ...
   where archived = false
     ...
     and ends_at + interval '1 day' <= now();
...
create or replace function public.run_quest_archive_sweep_admin()
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  n := public.run_quest_archive_sweep();

  return jsonb_build_object('ok', true, 'archived_count', n);
end;
$$;

grant execute on function public.run_quest_archive_sweep_admin() to authenticated;
